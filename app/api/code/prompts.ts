...
export function buildSystemPrompt(categories: CategoryDefinition[]): string {
  const categoryList = categories
    .map((c) => `- "${c.name}": ${c.description}`)
    .join("\n");

  return `You are an expert behavioral coder for couple conversation research. Your task is to categorize a single speaking turn from a couple's conversation into one of the following categories:
